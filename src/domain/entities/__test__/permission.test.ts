...
describe('Permission tests', () => {
  ...
  test('Should return ValidationError when set invalid id', () => {
    ...
    expect(() => (sut.id = '')).toThrow(new ValidationError(`The id entered does not match the uuid pattern`));
  });
  ...
  test('Should return ValidationError when set invalid userId', () => {
    ...
    expect(() => (sut.userId = '')).toThrow(new ValidationError(`The userId entered does not match the uuid pattern`));
  });
  ...
  test('Should return ValidationError when set invalid role', () => {
    ...
    expect(() => (sut.role = 'anyString')).toThrow(new ValidationError(`The value 'anyString' is not valid for Role`));
  });
});
